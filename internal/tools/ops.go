...
func registerOpsTools(s *mcp.Server, mgr *manager.Manager) {
	...
	mcp.AddTool(s, &mcp.Tool{
		Name:        "ping_host",
		Description: "Ping a destination from a device (read-only diagnostic).",
	...
	mcp.AddTool(s, &mcp.Tool{
		...
		Description: "Traceroute to a destination from a device (read-only diagnostic).",
	...
	mcp.AddTool(s, &mcp.Tool{
		...
		Description: "Routes advertised to a BGP neighbor ('show ip bgp neighbors <ip> advertised-routes').",
	...
	mcp.AddTool(s, &mcp.Tool{
		...
		Description: "Routes received from a BGP neighbor ('show ip bgp neighbors <ip> received-routes').",
	}, func(ctx context.Context, _ *mcp.CallToolRequest, args bgpRoutesArgs) (*mcp.CallToolResult, any, error) {
		if strings.TrimSpace(args.Neighbor) == "" {
			return nil, nil, fmt.Errorf("neighbor is required")
		}
